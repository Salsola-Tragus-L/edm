...
class GossipMechanism:
    def __init__(
        ...
        self._topology = topology
        self._w = topology.gossip_matrix(gossip_matrix) if gossip_matrix is not None else topology.gossip_matrix()
        self._message_drop_prob = message_drop_prob
        self._tag = tag
        self.bytes_sent = 0
...
class MultiTopologyGossipMechanism:
    ...
    Divide communication evenly over multiple topologies
    """
    def __init__(self, topologies, gossip_matrix=None, **kwargs):
        if not isinstance(topologies, list):
            topologies = [topologies]

        self._gossips = [GossipMechanism(t, gossip_matrix, **kwargs) for t in topologies]
