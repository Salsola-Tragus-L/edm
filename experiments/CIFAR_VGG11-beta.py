...
# mom_list = [0,0.1,0.2,0.4,0.6,0.8,0.9,0.95]
# mom_list = [0.97, 0.99]
mom_list = [0,0.1,0.2, 0.4,0.6,0.8,0.9,0.95]

# alg_list = ["d2", "gradient-tracking", "decent_lam"]
# alg_list = ["d2"]
# alg_list = ["edm", "quasi-global-momentum", "dmsgt_hb"]
alg = "edm"
for seed in [1]:                                      # [2,3]
    for mom in mom_list:
        for alpha in [1]:                               # [1, 0.1, 0.01]
            for topology in ["ring"]:                     # ['fully-connected'], ["ring"]
                lr = 0.5                                 # best_lrs[alpha, topology]
                config = {**base_config,
                          "learning_rate": lr,
                          "momentum": mom,
                          "topology": topology,
                ...
                logdir = "/root/autodl-tmp/relaysgd/logs-beta_test3-alpha1/"+job_name + '/'
                if not os.path.isdir(logdir):
                    os.makedirs(logdir)
                np.save(logdir+'config.npy', config)
                run_cmd('mpirun -np {} python /root/autodl-tmp/relaysgd/train-beta.py --path "{}"'.format(num_workers, logdir))
